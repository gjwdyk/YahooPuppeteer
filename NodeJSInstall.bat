...
rem Any %var% within the block will be replaced by that variable's value at the time the block is parsed - before the block is executed - the same thing applies to a FOR ... DO (block).
rem Hence, IF (something) else (somethingelse) will be executed using the values of %variables% at the time the IF is encountered.
rem
rem Two common ways to overcome this are :
rem (1) to use setlocal enabledelayedexpansion and use !var! in place of %var% to access the changed value of var
rem or
